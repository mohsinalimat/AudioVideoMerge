...
@interface ViewController ()<AVCaptureFileOutputRecordingDelegate, CLLocationManagerDelegate>
{
    NSTimer *timer;
    NSString *outputFileName;
}
@property (weak, nonatomic) IBOutlet AVMetadataRecordPlayCameraPreviewView *previewView;
...
- (IBAction)toggleMovieRecording:(id)sender
{
    ...
    dispatch_async( self.sessionQueue, ^{
        if ( ! self.movieFileOutput.isRecording ) {
            ...
            outputFileName = [NSUUID UUID].UUIDString;
            NSString *outputFilePath = [NSTemporaryDirectory() stringByAppendingPathComponent:[outputFileName stringByAppendingPathExtension:@"m4v"]];
    ...
    [self performSelector:@selector(stopRecordingVideo) withObject:self afterDelay:30.0];
}
...
//                    NSLog(@"Gallery Path : %@",outputFileURL.description);
//                    [[NSUserDefaults standardUserDefaults] setURL:outputFileURL forKey:@"videoURL"];
...
//            }
//        }];
        [self performSegueWithIdentifier:@"mergeScreen" sender:nil];
    ...
    dispatch_async( dispatch_get_main_queue(), ^{
        ...
        [self.recordButton setTitle:NSLocalizedString( @"RECORDING", @"Recording button record title") forState:UIControlStateNormal];
    });
}
...
// Pass the selected object to the new view controller.
    if ([[segue identifier] isEqualToString:@"mergeScreen"])
    {
        MergeScreenVC *mergeVC = [segue destinationViewController];
        mergeVC.strName = outputFileName;
    }
}
